...
def calculate_id_stats(df, name, id_col):
    ...
    stats = {
        ...
        f'% Non-Null {id_col}': round((non_null_ids / total_rows) * 100, 2) if total_rows > 0 else 0
    }
    return stats
...
if LISTINGS_ID_COL in df_listings.columns and CALENDAR_ID_COL in df_calendar.columns:
    ...
    listings_ids = pd.to_numeric(df_listings[LISTINGS_ID_COL], errors='coerce').dropna().unique()
    calendar_ids = pd.to_numeric(df_calendar[CALENDAR_ID_COL], errors='coerce').dropna().unique()

    # Create temporary DFs with a common column name for merging
    df_listings_ids = pd.DataFrame({CALENDAR_ID_COL: listings_ids}) # Rename listings ID col to match calendar
    ...
    print(f"Unique valid listing_ids in listings (col '{LISTINGS_ID_COL}'): {listings_unique_ids}")
    print(f"Unique valid listing_ids in calendar (col '{CALENDAR_ID_COL}'): {calendar_unique_ids}")
    print(f"Unique listing_ids present in BOTH listings and calendar (after cleaning): {merged_unique_ids}")

    if listings_unique_ids > 0:
        percentage_retained = round((merged_unique_ids / listings_unique_ids) * 100, 2)
        print(f"Percentage of unique listings retained after merge: {percentage_retained}%")
    else:
        print("Cannot calculate retention percentage, no valid unique listings found in listings df.")
else:
    print(f"Skipping merge simulation: ID column missing ('{LISTINGS_ID_COL}' in listings or '{CALENDAR_ID_COL}' in calendar).")