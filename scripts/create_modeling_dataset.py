...
def load_data(processed_data_dir):
    ...
    reliable_listings = pd.read_pickle(
        os.path.join(processed_data_dir, "analysis", "reliable_listings.pkl")
    )
...
def prepare_modeling_features(listings_df):
    ...
    for col in numeric_columns:
        if col in modeling_features.columns:
            modeling_features[col] = pd.to_numeric(
                modeling_features[col], errors="coerce"
            )
    ...
    if (
        "price" in modeling_features.columns
        and modeling_features["price"].dtype == "object"
    ):
        modeling_features["price_listings"] = modeling_features["price"].str.replace(
            r"[\$,]", "", regex=True
        )
        modeling_features["price_listings"] = pd.to_numeric(
            modeling_features["price_listings"], errors="coerce"
        )
    ...
    quality_report = {
        ...
        "geographic_coverage": {
            ...
            "coordinate_completeness": (
                ...
                if "latitude" in modeling_dataset.columns
                else 0
            ),
        },
    }
...
def main():
    ...
    print(
        f"Target range: £{quality_report['target_statistics']['annual_revenue_estimate']['min']:,.0f} - £{quality_report['target_statistics']['annual_revenue_estimate']['max']:,.0f}"
    )
    print(
        f"Target median: £{quality_report['target_statistics']['annual_revenue_estimate']['median']:,.0f}"
    )
