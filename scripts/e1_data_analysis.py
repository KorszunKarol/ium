...
def calculate_occupancy(processed_calendar_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "available_numeric" not in processed_calendar_df.columns:
        ...
        return None
    if (
        "date" not in processed_calendar_df.columns
        or not pd.api.types.is_datetime64_any_dtype(processed_calendar_df["date"])
    ):
        print(
            "Error: 'date' column (datetime type) required for occupancy calculation."
        )
        return None

    occupancy = (
        processed_calendar_df.groupby("listing_id")
        .agg(
    ...
    occupancy["occupancy_rate"] = occupancy.apply(
        lambda row: (
            row["booked_days"] / row["total_days"] if row["total_days"] > 0 else 0
        ),
        axis=1,
    )
...
def calculate_adr(processed_calendar_df: pd.DataFrame) -> pd.DataFrame:
    ...
    available_days = processed_calendar_df[
        processed_calendar_df["available_numeric"] == 1
    ].copy()
...
def visualize_neighbourhood_prices(listings_df: pd.DataFrame) -> folium.Map:
    ...
    if "latitude" in cleaned_df.columns and "longitude" in cleaned_df.columns:
        ...
        for idx, row in neighbourhood_prices.iterrows():
            ...
            neighborhood_listings = cleaned_df[
                cleaned_df["neighbourhood_cleansed"] == neighborhood
            ]
            if len(neighborhood_listings) > 0:
                ...
                folium.Marker(
                    ...
                    icon=folium.Icon(
                        color=(
                            "green"
                            if avg_price > neighbourhood_prices["avg_price"].median()
                            else "blue"
                        )
                    ),
    ...
    if processed_calendar_df is not None:
        ...
        if occupancy_df is not None:
            ...
            try:
                ...
                if "id" in listings_df.columns and not pd.api.types.is_numeric_dtype(
                    listings_df["id"]
                ):
                    listings_df["id"] = pd.to_numeric(
                        listings_df["id"], errors="coerce"
                    )
                ...
                listings_df["id"] = listings_df["id"].astype(
                    np.int64
                )  # Or appropriate int type
        ...
        if adr_df is not None:
            ...
            try:
                ...
                if "id" in listings_df.columns and pd.api.types.is_numeric_dtype(
                    listings_df["id"]
                ):  # Check if ID is usable
                    adr_df = adr_df.dropna(subset=["listing_id"])
                    adr_df["listing_id"] = adr_df["listing_id"].astype(
                    ...
                    if (
                        "listing_id" in listings_df.columns
                        and "id" in listings_df.columns
                        and "listing_id" != "id"
                    ):
