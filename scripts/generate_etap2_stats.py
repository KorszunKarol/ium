...
def generate_statistics(listings_df, calendar_df, reviews_df):
    ...
    three_way_overlap = len(listings_ids.intersection(calendar_ids).intersection(reviews_ids))
    ...
    critical_features = ['neighbourhood_cleansed', 'bedrooms', 'bathrooms_text', 'beds', 'latitude', 'longitude', 'price']
    missing_analysis = {}
    ...
    if 'date' in reviews_df.columns:
        ...
        if 'comments' in reviews_df.columns:
            ...
            stats['reviews_analysis']['avg_comment_length'] = non_null_comments.str.len().mean()
            stats['reviews_analysis']['missing_comments_count'] = reviews_df['comments'].isna().sum()
            stats['reviews_analysis']['missing_comments_pct'] = reviews_df['comments'].isna().sum() / len(reviews_df) * 100
