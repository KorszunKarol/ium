...
def calculate_listing_statistics(calendar_df):
    ...
    listing_stats["booked_days"] = (
        listing_stats["total_days"] - listing_stats["available_days"]
    )
    listing_stats["occupancy_rate"] = (
        listing_stats["booked_days"] / listing_stats["total_days"]
    )
...
def generate_summary_statistics(listing_stats):
    ...
    summary = {
        ...
        "observation_period_stats": {
            k: convert_to_json_serializable(v) for k, v in obs_stats.to_dict().items()
        },
        "occupancy_rate_stats": {
            k: convert_to_json_serializable(v) for k, v in occ_stats.to_dict().items()
        },
        "price_stats": {
            k: convert_to_json_serializable(v) for k, v in price_stats.to_dict().items()
        },
        "data_quality_metrics": {
            "listings_with_price_data": int(listing_stats["avg_price"].notna().sum()),
            "median_observation_period": float(
                listing_stats["observation_period_days"].median()
            ),
            "mean_occupancy_rate": float(listing_stats["occupancy_rate"].mean()),
            "listings_with_min_30_days": int(
                (listing_stats["observation_period_days"] >= 30).sum()
            ),
            "listings_with_min_20_price_records": int(
                (listing_stats["price_records"] >= 20).sum()
            ),
        },
    }
    return summary
...
def filter_reliable_listings(listing_stats, min_days=30, min_price_records=20):
    ...
    reliable_listings["annual_revenue_estimate"] = (
        reliable_listings["daily_revenue"] * 365
    )

    reliable_listings["confidence_score"] = np.minimum(
        reliable_listings["observation_period_days"] / 365, 1.0
    ) * np.minimum(reliable_listings["price_records"] / 100, 1.0)
...
def main():
    ...
    summary["reliable_listings_percentage"] = float(
        len(reliable_listings) / len(listing_stats) * 100
    )
    ...
    print(
        f"  Reliable listings: {int(summary['reliable_listings_count']):,} ({summary['reliable_listings_percentage']:.1f}%)"
    )
    print(
        f"  Median observation period: {summary['data_quality_metrics']['median_observation_period']:.0f} days"
    )
    print(
        f"  Mean occupancy rate: {summary['data_quality_metrics']['mean_occupancy_rate']:.3f}"
    )
