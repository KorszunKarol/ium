...
class AirbnbVisualizationGenerator:
    def __init__(self, data_dir='data/processed/etap2/', output_dir='reports/figures/etap2/'):
        """
    ...
    def load_data(self):
        ...
        try:
            self.reliable_listings = pd.read_pickle(self.data_dir + 'analysis/reliable_listings.pkl')
    ...
    def generate_missing_data_analysis(self):
        ...
        geo_review_available = [f for f in geo_review_features if f in self.listings_df.columns]
        ...
        if geo_review_available:
            ...
            plt.title('Missing Value Correlation: Geographic & Review Features',
    ...
    def generate_geographic_distribution(self):
        ...
        if 'latitude' not in self.listings_df.columns or 'longitude' not in self.listings_df.columns:
            print("Latitude or Longitude data not available")
            return

        # Interactive London Listings Geographic Distribution
        plot_df = self.listings_df[['latitude', 'longitude', 'room_type', 'price', 'neighbourhood_cleansed']].copy()
        plot_df.dropna(subset=['latitude', 'longitude'], inplace=True)

        # Clean price data
        if 'price' in plot_df.columns:
            plot_df['price_cleaned'] = plot_df['price'].replace({r'\$|,': ''}, regex=True)
            plot_df['price_cleaned'] = pd.to_numeric(plot_df['price_cleaned'], errors='coerce')
            # Fill NaN values with median price or a default value
            plot_df['price_cleaned'] = plot_df['price_cleaned'].fillna(plot_df['price_cleaned'].median())
        ...
        fig = px.scatter_mapbox(sample_df,
                                ...
                                hover_data={"latitude": False, "longitude": False,
                                          "room_type": True, "price_cleaned": ":.2f"},
    ...
    def generate_property_features(self):
        ...
        if 'accommodates' in self.listings_df.columns:
            plt.figure(figsize=(12, 6))
            accommodates_data = pd.to_numeric(self.listings_df['accommodates'], errors='coerce').dropna()
    ...
    def generate_review_scores_analysis(self):
        ...
        if 'review_scores_rating' in self.listings_df.columns:
            plt.figure(figsize=(12, 6))
            review_scores = pd.to_numeric(self.listings_df['review_scores_rating'], errors='coerce').dropna()
        ...
        if 'number_of_reviews' in self.listings_df.columns:
            plt.figure(figsize=(12, 6))
            review_counts = pd.to_numeric(self.listings_df['number_of_reviews'], errors='coerce').dropna()
    ...
    def generate_calendar_price_analysis(self):
        ...
        if 'price_cleaned' in self.calendar_df.columns:
            ...
            plt.hist(filtered_prices, bins=50, alpha=0.7, color='lightgreen',
                    edgecolor='black', linewidth=0.5)
            plt.xlabel('Price (£)')
    ...
    def generate_revenue_analysis(self):
        ...
        if not self.has_reliable_data or 'annual_revenue_estimate' not in self.reliable_listings.columns:
        ...
        plt.hist(revenue_data, bins=50, range=(0, upper_limit), alpha=0.7,
                color='darkblue', edgecolor='black', linewidth=0.5)
        plt.xlabel('Annual Revenue Estimate (£)')
        plt.ylabel('Number of Listings')
        plt.title('Distribution of Annual Revenue Estimates (View up to 95th Percentile)',
                 fontweight='bold', pad=20)
        plt.grid(alpha=0.3)
        ...
        plt.axvline(mean_revenue_in_view, color='red', linestyle='--',
                   label=f'Mean (View): £{mean_revenue_in_view:,.0f}')
        plt.axvline(median_revenue_in_view, color='blue', linestyle='--',
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Generate professional Airbnb data visualizations')
    parser.add_argument('--data_dir', default='data/processed/etap2/',
                       help='Path to processed data directory')
    parser.add_argument('--output_dir', default='reports/figures/etap2/',
